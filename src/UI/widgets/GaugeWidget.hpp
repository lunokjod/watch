...
class GaugeWidget : public CanvasWidget, ActiveRect {
    public:
        ...
        int32_t markY = -1;
        void ForceDraw();
        GaugeWidget(int16_t x, int16_t y, int16_t d=100, uint32_t wgaugeColor=ThCol(button), uint32_t markColor=ThCol(middle), uint32_t needleColor=ThCol(mark), uint32_t incrementColor=ThCol(max));
