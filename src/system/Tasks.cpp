...
bool RemoveTimedTask(TimedTaskDescriptor *oldTsk) {
    lLog("RemoveTimedTask: Task %p '%s' removed\n", oldTsk,oldTsk->name);
...
bool AddTimedTask(TimedTaskDescriptor *nuTsk) {
    ...
    for (auto const& tsk : pendingTasks) {
        if ( tsk == nuTsk ) {
            lLog("AddTimedTask: Task %p '%s' already on the list (offset: %d)\n", tsk,tsk->name, offset);
            return false;
        }
        offset++;
    }
    pendingTasks.push_back(nuTsk);
    lLog("AddTimedTask: Task %p '%s' added (offset: %d)\n", nuTsk,nuTsk->name, offset);
    return true;
}
...
static void TimedHandlerTask(void* args) {
    ...
    while(true) {
        ...
        if ( false == ntpSyncDone ) {
            lLog("TimedTasks: Not runnig due no NTP sync or manual set timedate isn't done\n");
            delay(10*1000);
            continue;
        }
        ...
        for (auto const& tsk : pendingTasks) {
            if ( -1 == tsk->_nextTrigger ) {
                ...
                if ( millis() > tsk->_nextTrigger ) {
                    delay(150);
                    lLog("TimedTasks: Running task '%s'...\n", tsk->name);
                    bool res = tsk->callback();
                    if ( res ) {
                        tsk->_nextTrigger = millis()+tsk->everyTimeMS;
                        tsk->_lastCheck = millis();
                        lLog("TimedTasks: Task '%s' done!\n", tsk->name);
                    } else { 
                        lLog("TimedTasks: Task '%s' FAILED!\n", tsk->name);
                    }
                }
            }
        }        
